    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', marginBottom: '14px' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--win-home)' }} /> 直接出线 (前二)
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--draw)' }} /> 小组第三 (争最佳8席)
        </span>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
              <thead>
                <tr style={{ color: 'var(--text-muted)', fontSize: '0.7rem', textAlign: 'left' }}>
                  <th style={{ padding: '2px 4px', fontWeight: 500 }}>球队</th>
                  <th style={{ padding: '2px 4px', fontWeight: 500, textAlign: 'right' }}>均分</th>
                  <th style={{ padding: '2px 4px', fontWeight: 500, textAlign: 'right' }}>出线率</th>
                </tr>
              </thead>
              <tbody>
                {groups[label].map((t, rank) => (
                  <tr key={t.name} style={{ borderTop: '1px solid var(--border-light)' }}>
