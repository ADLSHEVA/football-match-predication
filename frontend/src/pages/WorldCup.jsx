...
// `text` holds the Chinese key; translated at render via t().
const SOURCE_LABELS = {
...
export default function WorldCup() {
  ...
  const load = useCallback(async (refresh = false, n = sims) => {
    ...
    try {
      ...
      if (!res.ok) {
        let msg = `${t('服务器错误 (HTTP ')}${res.status})`;
      ...
      console.error(e);
      setErrorMsg(e.message || t('无法连接到后端 API。'));
    } finally {
      setLoading(false);
    }
  }, [sims, t]);
    ...
    <div className="dashboard-main-wrapper">
      ...
      <div className="section-header" style={{ marginBottom: '20px' }}>
        <div>
          ...
          <span className="section-subtitle">
            {t('48 强 / 12 组 · 基于 Elo 实力先验的蒙特卡洛全赛事推演（小组赛 → 淘汰赛 → 夺冠）。')}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          ...
          >
            {SIM_OPTIONS.map((n) => (
              <option key={n} value={n}>{n.toLocaleString()} {t('次模拟')}</option>
            ))}
          </select>
          <button className="btn-action" onClick={handleRefresh} disabled={loading} title={t('刷新实时数据并重新模拟')}>
      ...
      {data && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', marginBottom: '16px', fontSize: '0.8rem' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            {t('数据源: ')}<strong style={{ color: sourceMeta.color }}>{t(sourceMeta.text)}</strong>
          </span>
          <span style={{ color: 'var(--text-muted)' }}>{t('已录入赛果: ')}<strong style={{ color: 'var(--text-secondary)' }}>{data.matches_played}</strong>{t(' 场')}</span>
          <span style={{ color: 'var(--text-muted)' }}>{t('模拟次数: ')}<strong style={{ color: 'var(--text-secondary)' }}>{data.num_simulations?.toLocaleString()}</strong></span>
        </div>
      )}
      ...
      {errorMsg && (
        <div className="alert-banner warning" style={{ marginBottom: '16px' }}>
          ...
          <button className="btn-action" onClick={() => load(false, sims)} style={{ padding: '4px 10px' }}>{t('重试')}</button>
        </div>
      )}
      {data && data.source === 'seed' && !errorMsg && (
        <div className="alert-banner info" style={{ marginBottom: '16px' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {t('当前使用内置 2026 种子数据（分组为示意，非官方抽签）。配置 FOOTBALL_DATA_API_KEY 后点击「刷新」即可接入真实赛程与赛果。')}
          </span>
        </div>
      )}

      {/* Section tabs */}
      <div className="navigation-tabs" style={{ display: 'inline-flex', maxWidth: '100%', overflowX: 'auto', marginBottom: '20px' }}>
        <button className={`nav-tab ${section === 'odds' ? 'active' : ''}`} onClick={() => setSection('odds')}>
          <Trophy size={15} /> {t('夺冠概率')}
        </button>
      ...
      {section === 'live' ? (
        ...
        <div className="glass-panel loading-spinner-overlay" style={{ height: '300px' }}>
          ...
          <span><Cpu size={14} className="animate-spin" /> {t('正在推演')} {sims.toLocaleString()} {t('届世界杯...')}</span>
        </div>
      ) : data ? (
        <>
