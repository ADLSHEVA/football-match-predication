...
export default function Dashboard({
  ...
  const [logText, setLogText] = useState(t('系统就绪。点击同步或训练按钮开始。\n'));
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(e);
      setErrorMsg(t('无法连接到 API 后端，请确保 FastAPI 服务器已正常启动。'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSync = async () => {
    setSyncLoading(true);
    setLogText((prev) => prev + `[${ts()}] ${t('开始同步')} ${activeLeague} ${t('联赛数据...')}\n`);
    try {
      ...
      if (res.ok) {
        setLogText((prev) => prev + `[${ts()}] ${t('同步成功！新增比赛/xG数据: ')}${JSON.stringify(data.report)}\n`);
        if (data.report?.warning) {
          setLogText((prev) => prev + `[${ts()}] ${t('提示: ')}${data.report.warning}\n`);
        }
      } else {
        setLogText((prev) => prev + `[${ts()}] ${t('同步失败: ')}${data.detail}\n`);
      }
    } catch (e) {
      setLogText((prev) => prev + `[${ts()}] ${t('同步出错: ')}${e.message}\n`);
    } finally {
  ...
  const handleManualPredict = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setManualPredResult({ error: err.detail || t('预测失败') });
      }
    } catch (e) {
  ...
  const handleTrain = async () => {
    ...
    setLogText((prev) => prev + `[${ts()}] ${t('开始基于历史 xG 数据拟合 Dixon-Coles 模型与 ELO...')}\n`);
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setLogText((prev) => prev + `[${ts()}] ${t('模型训练完成！\n')}` +
          `${t('- 拟合球队数量: ')}${data.teams_fitted}\n` +
          `${t('- 主场优势因子 (Home Adv): ')}${data.home_advantage?.toFixed(4)}\n` +
          `${t('- 低进球相关修正 (Rho): ')}${data.rho?.toFixed(4)}\n`);
        setModelFitted(true);
      } else {
        setLogText((prev) => prev + `[${ts()}] ${t('训练失败: ')}${data.detail}\n`);
      }
    } catch (e) {
      setLogText((prev) => prev + `[${ts()}] ${t('训练出错: ')}${e.message}\n`);
    } finally {
    ...
    <div className="dashboard-main-wrapper">
      ...
      {errorMsg && (
        <div className="alert-banner warning">
          ...
          <button className="btn-action" onClick={fetchData} style={{ padding: '4px 10px' }}>{t('重试')}</button>
        </div>
      )}
      ...
      {!errorMsg && !modelFitted && (
        <div className="alert-banner info">
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {t('系统警告: ')}{activeLeague} {t('联赛的 Dixon-Coles 实力估值模型尚未训练！请在右侧控制面板进行「同步」与「训练」。')}
          </span>
        </div>
      )}
      ...
      {!errorMsg && !apiKeyConfigured && (
        <div className="alert-banner warning">
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <AlertTriangle size={18} />
            {t('未配置 Football-Data.org API Key。历史数据将从免费CSV源加载，但无法获取未来赛程。请在项目根目录 .env 文件中设置 FOOTBALL_DATA_API_KEY（免费注册: https://www.football-data.org/）')}
          </span>
        </div>
      )}
          ...
          <div className="section-header">
            <div>
              <h2 className="section-title">
                ...
                {t('赛程与胜率预测 (Dixon-Coles 基准)')}
              </h2>
              <span className="section-subtitle">{t('展示未来即将进行的比赛。点击「模拟微调」进入星蜥定量面板。')}</span>
            </div>
            <button
          ...
          {loading ? (
              ...
              {predictions.map((match) => (
                <div key={match.match_id} className="glass-panel match-card">
                  <div className="match-meta">
                    ...
                    <span>{match.date ? new Date(match.date).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : t('未定')}</span>
                    {batchOdds[`${match.home_team}|${match.away_team}`] && (
                      <span className="value-badge positive">{t('赔率可用')}</span>
                    )}
                  </div>
                  ...
                  <div style={{ marginTop: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                      <span>{t('Dixon-Coles 胜率:')}</span>
                      <span>{t('均值: ')}{match.expected_home_goals?.toFixed(1)} - {match.expected_away_goals?.toFixed(1)}</span>
                    </div>
                    <PredictionGauge
                  ...
                  <div className="match-footer">
                    <span className="elo-comparison">
                      {t('ELO 胜率: ')}{(match.elo_home_win * 100).toFixed(0)}{t('% 主胜')}
                    </span>
                    <button
                      className="btn-action"
            ...
            <div>
              <div className="alert-banner warning" style={{ marginBottom: '16px' }}>
                {t('已获取赛程，但由于实力估值模型未训练，无法显示胜率预测。请在右侧控制面板点击「同步」和「拟合模型」。')}
              </div>
              <div className="matches-grid">
                {rawMatches.map((match) => (
                  <div key={match.id} className="glass-panel match-card">
                    ...
                    <div className="match-footer">
                      <span className="elo-comparison">{t('基准数据未加载')}</span>
                      <button
                        className="btn-action"
          ...
          <div className="glass-panel">
            ...
            <div className="control-panel">
              <div className="status-indicator">
                ...
                <span>{t('当前联赛实力估值模型: ')}{modelFitted ? t('已训练 (Fitted)') : t('未准备 (Not Fitted)')}</span>
              </div>
              ...
              >
                <Database size={16} />
                {syncLoading ? t('正在同步数据...') : t('1. 同步英超/西甲数据')}
              </button>
              ...
              >
                <Cpu size={16} />
                {trainLoading ? t('正在训练拟合中...') : t('2. 拟合 Dixon-Coles 模型')}
              </button>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{t('系统实时运行日志:')}</span>
            ...
            <div className="glass-panel" style={{ background: 'linear-gradient(135deg, rgba(30, 41, 59, 0.45), rgba(6, 182, 212, 0.05))' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{t('模型回测预测总数:')}</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: '700' }}>{accuracy.total_predictions}{t(' 场')}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{t('主平客准确预测:')}</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: '700', color: 'var(--win-home)' }}>{accuracy.correct}{t(' 场')}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-light)', paddingTop: '10px' }}>
                  <span style={{ fontSize: '0.9rem', fontWeight: '600' }}>{t('总体基准胜率 (Hit Rate):')}</span>
                ...
                <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', lineHeight: '1.3' }}>
                  {t('* 注: 对于三项胜率预测(主胜/平局/客胜)，命中率高于 52% 即可跑赢庄家，55% 以上属于专业基金量化水平。')}
                </span>
              </div>
            </div>
          )}
