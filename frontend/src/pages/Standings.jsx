...
export default function Standings({ activeLeague, setActiveLeague }) {
  ...
  const fetchStandings = async () => {
    ...
    try {
      ...
      if (!standRes.ok) {
        throw new Error(t('无法获取积分榜数据，可能是 API Key 达到限制或网络问题'));
      }
      ...
      if (teamsRes.ok) {
        ...
        teamsData.teams.forEach(tm => {
          paramsMap[tm.name] = tm;
        });
      }
        ...
        <div className="section-header">
          <div>
            ...
            <span className="section-subtitle">
              {t('结合实时联赛积分，与 Dixon-Coles 拟合进攻/防守实力 (α, β) 和当前 ELO。')}
            </span>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="standings-container">
            <table className="data-table">
              <thead>
                <tr>
                  <th className="rank-cell">{t('排名')}</th>
                  <th>{t('球队')}</th>
                  <th style={{ textAlign: 'center' }}>{t('已赛')}</th>
                  <th style={{ textAlign: 'center' }}>{t('胜/平/负')}</th>
                  <th style={{ textAlign: 'center' }}>{t('进/失/净')}</th>
                  <th style={{ textAlign: 'center' }}>{t('积分')}</th>
                  <th style={{ textAlign: 'center', color: 'var(--accent)' }}>{t('ELO 评分')}</th>
                  <th style={{ textAlign: 'center', color: 'var(--win-home)' }}>{t('进攻强度 (α)')}</th>
                  <th style={{ textAlign: 'center', color: 'var(--win-away)' }}>{t('防守强度 (β)')}</th>
                </tr>
              </thead>
              <tbody>
                {standings.map((row) => {
                    ...
                    <tr key={row.team_id || row.team}>
                      ...
                      <td style={{ textAlign: 'center', color: 'var(--win-home)', fontFamily: 'var(--font-mono)' }}>
                        {param.attack ? (
                          ...
                          <span style={{ opacity: 0.35 }}>{t('未拟合')}</span>
                        )}
                      </td>
